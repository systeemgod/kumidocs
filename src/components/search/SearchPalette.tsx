		...
		<CommandList>
			...
			{activeResults.length > 0 && (
				<CommandGroup heading="Pages">
					{activeResults.map((result): JSX.Element => (
						<CommandItem key={result.path} value={result.path} onSelect={(): void => { onSelect(result.path); }} className="gap-2">
							<span className="shrink-0">
								<EmojiIcon emoji={result.emoji} fileType={result.type ?? 'doc'} size={EMOJI_SIZE} />
							</span>
							<div className="flex flex-col min-w-0">
								<span className="font-medium text-sm">{result.title}</span>
								<span className="text-xs text-muted-foreground truncate">{result.snippet}</span>
							</div>
							<span className="ml-auto text-xs text-muted-foreground shrink-0">{result.path}</span>
						</CommandItem>
					))}
				</CommandGroup>
			)}
...
const SearchPalette = (allProps: SearchPaletteProps): JSX.Element => {
	...
	const handleSelect = (path: string): void => {
		...
		if (ext === 'md') { navPath = `/p/${path}`; }
		navigate(navPath);
	};

	return (
		<CommandDialog open={open} onOpenChange={(isOpen: boolean): void => {
			if (!isOpen) { setQuery(''); setResults([]); onClose(); }
		}} shouldFilter={false}>
			<CommandInput placeholder="Search pages..." value={query} onValueChange={setQuery} />
			<SearchResultsList loading={loading} query={query} results={results} onSelect={handleSelect} />
		</CommandDialog>
	);
};
