...
const buildOuterStyle = (
	...
	if (directives.color) { (style as Record<string, unknown>)['--slide-fg'] = directives.color; }
	if (padding) {
...
const SlideMarkdownViewerInner = (allProps: SlideMarkdownViewerProps): JSX.Element => {
		...
		return (
			<div className="flex h-full overflow-hidden" style={outerStyle}>
				<div className="flex-1 overflow-hidden">
					<div className={cn(PROSE_BASE, 'px-6 py-5')}><SlideStreamdown value={left} /></div>
				</div>
				<div className="w-px shrink-0 bg-current opacity-15" />
				<div className="flex-1 overflow-hidden">
					<div className={cn(PROSE_BASE, 'px-6 py-5')}><SlideStreamdown value={right} /></div>
				</div>
			</div>
		);
	}
	if (isCenter) {
		return (
			<div className="h-full flex flex-col items-center justify-center text-center overflow-hidden" style={outerStyle}>
				<div className={cn(PROSE_BASE, 'px-12 py-8', isTitle && 'slide-prose-title', isSection && 'slide-prose-section')}>
