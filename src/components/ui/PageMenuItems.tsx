...
const CreateSection = (allProps: CreateSectionProps): JSX.Element => {
	...
	if (variant !== 'dropdown') { Item = ContextMenuItem as typeof DropdownMenuItem; }
	let Sep: typeof DropdownMenuSeparator = DropdownMenuSeparator;
	if (variant !== 'dropdown') { Sep = ContextMenuSeparator as typeof DropdownMenuSeparator; }
	return (
		<>
			{onNewSubPage && (
				<Item onClick={() => { onNewSubPage(path.replace(/\.md$/iu, '')); }}>
					<AddRegular className="mr-2 w-4 h-4" />
					New subpage
				</Item>
			)}
			{onNewPage && (
				<Item onClick={() => { onNewPage(parentDir); }}>
		...
		<>
			...
			{onDelete && (
				<Item
					...
					onClick={() => { onDelete(path, displayTitle); }}
				>
					<Square className="mr-2 w-4 h-4 opacity-0" />
					Delete
				</Item>
...
const PageMenuItems = (allProps: PageMenuItemsProps): JSX.Element => {
	const { variant, href, path, displayTitle, isVirtual = false,
		parentDir = '', onNewSubPage, onNewPage, onDuplicate,
		onExportPdf, onMove, onDelete } = allProps;
	let Item: typeof DropdownMenuItem = DropdownMenuItem;
	if (variant !== 'dropdown') { Item = ContextMenuItem as typeof DropdownMenuItem; }

	if (isVirtual) {
		return (<Item asChild><Link to={href}>Create this page</Link></Item>);
	}
	...
	return (
		<>
			{showCreateGroup && (
				<CreateSection variant={variant} path={path} parentDir={parentDir}
					onNewSubPage={onNewSubPage} onNewPage={onNewPage} onDuplicate={onDuplicate} />
			)}
			<Item onClick={() => { globalThis.open(href, '_blank'); }}>
				<OpenRegular className="mr-2 w-4 h-4" />
				Open in new tab
			</Item>
			<Item onClick={() => {
				navigator.clipboard.writeText(globalThis.location.origin + href)
			...
			{showDangerousGroup && (
				<DangerousSection variant={variant} path={path} displayTitle={displayTitle}
