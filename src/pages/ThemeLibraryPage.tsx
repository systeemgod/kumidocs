...
const STANDARD_LAYOUTS = new Set([
	'title', 'section', 'split', 'center', 'invert', 'blank', 'default',
]);
		...
		<button type="button" className="flex flex-col gap-2 text-left group" onClick={onClick}>
			<div className="rounded-md overflow-hidden border border-border shadow-sm group-hover:border-primary/50 group-hover:shadow-md transition-[box-shadow,border-color]">
				<div style={{ width: CARD_W, height: CARD_H, overflow: 'hidden', position: 'relative' }}>
					<ScaledSlide slide={titleSlide} scale={CARD_SCALE} theme={id} paginate={false} slideNum={1} total={1} slideThemes={slideThemes} origin="top left" />
				</div>
			</div>
			<div className="flex items-start gap-2">
				...
				{custom && <Badge variant="secondary" className="shrink-0 text-xs">custom</Badge>}
			</div>
		</button>
	);
};
...
const ThemeDialog = (allProps: ThemeDialogProps): JSX.Element => {
	...
	return (
		<Dialog open={open} onOpenChange={(isOpen: boolean): void => { if (!isOpen) { onClose(); } }}>
			<DialogContent showCloseButton={false} className="max-w-5xl w-full p-0 gap-0 overflow-hidden flex flex-col h-[85vh]">
				<div className="flex items-center justify-between px-4 py-3 border-b shrink-0">
					<div className="flex items-center gap-2">
						{active && <span className="text-sm font-semibold">{active.name}</span>}
						{active && active.custom && <Badge variant="secondary" className="text-xs">custom</Badge>}
					</div>
					<button type="button" onClick={onClose} className="rounded-sm opacity-70 hover:opacity-100 transition-opacity focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring">
						<X className="size-4" />
						<span className="sr-only">Close</span>
					</button>
				</div>
				<div className="flex-1 min-h-0">
					{active && <SlideViewer value={demoMarkdown} theme={active.id} slideThemes={slideThemes} paginate />}
				</div>
			</DialogContent>
		</Dialog>
	);
...
const ThemesGrid = (allProps: ThemesGridProps): JSX.Element => {
	...
	return (
		<div className="flex-1 overflow-y-auto p-6">
			<h2 className="text-xs font-semibold text-muted-foreground uppercase tracking-wide mb-4">Built-in</h2>
			<div className="grid gap-6 mb-8" style={colStyle}>
				{BUILTIN_THEMES.map((theme): JSX.Element => (
					<ThemeCard key={theme.id} id={theme.id} name={theme.name} description={theme.description} slideThemes={slideThemes}
						onClick={(): void => { onSelect({ id: theme.id, name: theme.name }); }} />
				))}
			</div>
			{customEntries.length > 0 && (
				<>
					<h2 className="text-xs font-semibold text-muted-foreground uppercase tracking-wide mb-4">Custom (.kumidocs.json)</h2>
					<div className="grid gap-6" style={colStyle}>
						{customEntries.map(([id, def]): JSX.Element => (
							<ThemeCard key={id} id={id} name={id} description={def.bg ?? 'Custom theme'} custom={def} slideThemes={slideThemes}
								onClick={(): void => { onSelect({ id, name: id, custom: def }); }} />
						))}
					</div>
				</>
			)}
...
const ThemeLibraryPage = (): JSX.Element => {
	...
	if (total === 1) { themeLabel = 'theme'; }
	let demoMarkdown = '';
	if (active) { demoMarkdown = generateDemoMarkdown(active.name, active.custom); }
	const handleSelect = (item: ActiveItem): void => { setActive(item); setDialogOpen(true); };
	return (
		<div className="flex flex-col h-full">
			<div className="flex items-center gap-3 px-6 py-4 border-b border-border shrink-0">
				<h1 className="text-xl font-semibold flex-1">Theme Library</h1>
				<span className="text-sm text-muted-foreground">{total} {themeLabel}</span>
			</div>
			<ThemesGrid slideThemes={slideThemes} customEntries={customEntries} onSelect={handleSelect} />
			<ThemeDialog active={active} open={dialogOpen} onClose={(): void => { setDialogOpen(false); }} slideThemes={slideThemes} demoMarkdown={demoMarkdown} />
		</div>
	);
};
