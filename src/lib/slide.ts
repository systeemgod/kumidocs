...
const isBgDark = (color: string): boolean => {
	...
	if (hexMatch) {
		...
		return RED_LUMINANCE * redChannel + GREEN_LUMINANCE * greenChannel + BLUE_LUMINANCE * blueChannel < DARK_THRESHOLD;
	}
	const oklchMatch = /oklch\(\s*([\d.]+)/u.exec(color);
	if (oklchMatch) { return Number.parseFloat(oklchMatch.at(1) ?? '1') < OKLCH_DARK_LIGHTNESS; }
	return false;
};
...
export { parseSlideDirectives, isBgDark, BUILTIN_SLIDE_THEMES, resolveCustomTheme, resolveTheme, splitAtSecondH2 };
